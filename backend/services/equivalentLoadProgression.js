function calculateProgressionPlan(currentReps, currentWeight) {
    const repPercentages = {
        15: 0.65, 14: 0.675, 13: 0.70, 12: 0.725,
        11: 0.75, 10: 0.775, 9: 0.80, 8: 0.825,
        7: 0.85, 6: 0.875
    };
    ...
    Object.keys(repPercentages).forEach(repsString => {
        ...
        if ((!uniqueWeights.has(practicalWeight) || uniqueWeights.get(practicalWeight) > reps) && 
    ...
    uniqueWeights.forEach((reps, weight) => {
        if (!(weight === currentWeight && reps === currentReps)) { // Exclude current session
